...
- **🚀 进程管理 V2**：
  ...
  - **平台管控**：支持一键重启或关闭整个平台（及所有子工具），方便更新维护。
...
首次运行新版目录结构时，旧的 `data/`、`logs/` 会尽力迁移到 `.local/`。
部署目标请参考 `deploy/targets.example.json`，写入 `.local/deploy/targets.json`；
通用部署流程始终保留在 `deploy/`，机器地址和路径不写入仓库。
